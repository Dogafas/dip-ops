...
- [Инфраструктура, манифесты и оркестрация (dip-ops)](https://github.com/Dogafas/dip-ops)
- [Исходный код приложения и CI/CD Pipeline (dip-app)](https://github.com/Dogafas/dip-app)
- [Реестр контейнеров Docker Hub (datadynamo/dip-app)](https://hub.docker.com/r/datadynamo/dip-app)
...
3. Публикация артефактов автоматизирована в публичном реестре Docker Hub: [datadynamo/dip-app](https://hub.docker.com/r/datadynamo/dip-app). Все релизы версионируются в соответствии с SemVer.
...
2. **Этап CD (публикация Git-тега `v*.*.*`):**
   - Извлечение версии релиза и автоматическое приведение тега к формату SemVer (с удалением префикса `v`).
   - Сборка и публикация неизменяемого релизного образа (например, `datadynamo/dip-app:1.0.6`).
...
| :--- | :--- | :--- |
| **Репозиторий dip-ops** | [https://github.com/Dogafas/dip-ops](https://github.com/Dogafas/dip-ops) | IaC, Kubespray, Ingress, Monitoring |
| **Репозиторий dip-app** | [https://github.com/Dogafas/dip-app](https://github.com/Dogafas/dip-app) | Исходный код, Dockerfile, GitHub Actions |
| **Реестр Docker Hub** | [https://hub.docker.com/r/datadynamo/dip-app](https://hub.docker.com/r/datadynamo/dip-app) | Релизные теги `1.0.0` — `1.0.6` |
...
### 7. Скриншоты выполнения этапов

#### 7.1. Реестр контейнеров Docker Hub
Публикация релизного образа с тегом `1.0.6`:
![Скриншот Docker Hub с релизным тегом 1.0.6](image-1.png)
*Рис. 1. Образ datadynamo/dip-app:1.0.6 в публичном реестре Docker Hub.*

#### 7.2. Успешный запуск CI/CD пайплайна (GitHub Actions)
Отработка стадий сборки и деплоя релиза `v1.0.6`:
![Скриншот выполнения GitHub Actions для релиза v1.0.6](image-6.png)
*Рис. 2. Успешное выполнение джоб build-and-push и deploy для тега v1.0.6.*

#### 7.3. Доступность приложения через Ingress
Открытие веб-страницы по доменному имени `nip.io`:
![Скриншот работы тестового приложения](image-2.png)
*Рис. 3. Ответ тестового приложения dip-app на 80 порту мастера через Ingress.*

#### 7.4. Панель мониторинга Grafana
Дашборд утилизации ресурсов кластера:
![Скриншот дашборда Grafana](image-4.png)
*Рис. 4. Метрики состояния узлов и подов кластера в веб-интерфейсе Grafana.*

#### 7.5. Состояние узлов и подов кластера Kubernetes
Контрольный вывод статуса всех компонентов в терминале:
![Скриншот статуса узлов и всех подов](image-5.png)
*Рис. 5. Вывод команд `kubectl get nodes -o wide` и `kubectl get pods -A` (все ноды Ready, поды Running).*
...
| Секрет | Описание |
| :--- | :--- |
| `DOCKERHUB_USERNAME` | Имя пользователя в реестре Docker Hub (`datadynamo`) |
...
> **Примечание:** При использовании утилиты `gh` авторизация и синхронизация секрета `KUBE_CONFIG` выполняются автоматически целью `make apps-up`. Если вы форкаете проект под своей учетной записью, убедитесь, что в переменной окружения `IMAGE_NAME` файла `.github/workflows/ci-cd.yml` указано ваше пространство имен на Docker Hub.